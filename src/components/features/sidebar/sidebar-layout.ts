...
/** Open WebUI-style navigation rows: quiet at rest, clear on hover/selection. */
...
export const SIDEBAR_HEADER_ROW_CLASS =
  "flex h-12 min-h-12 shrink-0 items-center gap-2 pl-3 pr-3 w-full";

export function sidebarHeaderRowClassName(collapsed: boolean): string {
  return cn(
    "flex h-12 min-h-12 shrink-0 items-center w-full",
    collapsed ? "px-0" : "gap-2 pl-3 pr-3",
  );
}

export const SIDEBAR_ROW_INTERACTIVE_CLASS = {
  active:
    "bg-[var(--oh-interactive-active)] text-[var(--oh-foreground)] font-normal",
  idle: "text-[var(--oh-text-secondary)] hover:text-[var(--oh-foreground)] hover:bg-[var(--oh-interactive-hover-low)]",
} as const;
...
export function sidebarNavRowClassName(options?: {
  ...
  return cn(
    "flex h-10 min-h-10 min-w-0 items-center rounded-xl",
